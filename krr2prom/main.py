...
def set_metric(metric, value, *labels):
    try:
        print(metric, labels, value, float(value))
...
def collect_metrics(result):
    ...
    for scan in result.scans:
        ...
        set_metric(KRR_RECOMMENDED_REQUESTS_CPU,    recom.requests['cpu'],      kind, name, container, namespace, severity(recom.requests['cpu']))
        set_metric(KRR_RECOMMENDED_REQUESTS_MEMORY, recom.requests['memory'],   kind, name, container, namespace, severity(recom.requests['memory']))
        set_metric(KRR_RECOMMENDED_LIMITS_CPU,      recom.limits['cpu'],        kind, name, container, namespace, severity(recom.limits['cpu']))
        set_metric(KRR_RECOMMENDED_LIMITS_MEMORY,   recom.limits['memory'],     kind, name, container, namespace, severity(recom.limits['memory']))
